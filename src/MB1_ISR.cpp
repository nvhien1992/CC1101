...
/**< EXTI0 sub ISR table */
void (* EXTI0_subISR_table [numOfSubISR_max]) (void);
/**< EXTI0 sub ISR table */

/**< EXTI1 sub ISR table */
void (* EXTI1_subISR_table [numOfSubISR_max]) (void);
/**< EXTI1 sub ISR table */
...
ISRMgr :: ISRMgr (void){
    ...
    EXTI0_subISR_table_init();
    EXTI1_subISR_table_init();
...
status_t ISRMgr::subISR_assign (ISR_t ISR_type, void (* subISR_p)(void) ){
    ...
    case ISRMgr_USART1:
        ...
        break;
    case ISRMgr_EXTI0:
    	retval = subISR_EXTI0_assign (subISR_p);
    	break;
    case ISRMgr_EXTI1:
    	retval = subISR_EXTI1_assign (subISR_p);
    	break;
    default:
        break;
    }
...
switch (ISR_type){
    ...
    case ISRMgr_USART1:
        ...
        break;
    case ISRMgr_EXTI0:
    	retval = subISR_EXTI0_remove (subISR_p);
    	break;
    case ISRMgr_EXTI1:
    	retval = subISR_EXTI1_remove (subISR_p);
    	break;
    default:
        break;
    }
...
/**< EXTI0 private */
/**
  * @brief subISR_EXTI0_assign. assign a sub ISR func ptr to EXTI0_subISR_table.
  * @param void (* subISR_p)(void)
  * @return None.
  */
status_t ISRMgr::subISR_EXTI0_assign (void (* subISR_p)(void)) {
    uint8_t i;
    bool found = false;

    for (i = 0; i < numOfSubISR_max; i++){
        if (EXTI0_subISR_table [i] == NULL){
            found = true;
            EXTI0_subISR_table [i] = subISR_p;
...
/**
  * @brief subISR_EXTI0_remove. Remove a sub ISR func ptr to EXTI0_subISR_table.
  * @param void (* subISR_p)(void)
  * @return None.
  */
status_t ISRMgr::subISR_EXTI0_remove (void (* subISR_p)(void)) {
    uint8_t i;
    bool found = false;

    for (i = 0; i < numOfSubISR_max; i++){
        if (EXTI0_subISR_table [i] == subISR_p){
            found = true;
            EXTI0_subISR_table [i] = NULL;
...
/**
  * @brief EXTI0_subISR_table_init. Init all value in EXTI0_subISR_table.
  * @return None.
  */
void ISRMgr::EXTI0_subISR_table_init (void){
    uint8_t a_count;
    for (a_count = 0; a_count < numOfSubISR_max; a_count++ ){
    	EXTI0_subISR_table [a_count] = NULL;
    }
}
...
/**< EXTI1 private */
/**
  * @brief subISR_EXTI1_assign. assign a sub ISR func ptr to EXTI1_subISR_table.
  * @param void (* subISR_p)(void)
  * @return None.
  */
status_t ISRMgr::subISR_EXTI1_assign (void (* subISR_p)(void)) {
    uint8_t i;
    bool found = false;

    for (i = 0; i < numOfSubISR_max; i++){
        if (EXTI1_subISR_table [i] == NULL){
            found = true;
            EXTI1_subISR_table [i] = subISR_p;
...
/**
  * @brief subISR_EXTI1_remove. Remove a sub ISR func ptr to EXTI1_subISR_table.
  * @param void (* subISR_p)(void)
  * @return None.
  */
status_t ISRMgr::subISR_EXTI1_remove (void (* subISR_p)(void)) {
    uint8_t i;
    bool found = false;

    for (i = 0; i < numOfSubISR_max; i++){
        if (EXTI1_subISR_table [i] == subISR_p){
            found = true;
            EXTI1_subISR_table [i] = NULL;
...
/**
  * @brief EXTI1_subISR_table_init. Init all value in EXTI1_subISR_table.
  * @return None.
  */
void ISRMgr::EXTI1_subISR_table_init (void){
    uint8_t a_count;
    for (a_count = 0; a_count < numOfSubISR_max; a_count++ ){
    	EXTI1_subISR_table [a_count] = NULL;
    }
}
...
void EXTI0_IRQHandler (void) {
    ...
    for (a_count = 0; a_count < numOfSubISR_max; a_count++){
        if (EXTI0_subISR_table[a_count] != NULL){
        	EXTI0_subISR_table[a_count] ();
        }
    }
...
void EXTI1_IRQHandler (void) {
    ...
    for (a_count = 0; a_count < numOfSubISR_max; a_count++){
        if (EXTI1_subISR_table[a_count] != NULL){
        	EXTI1_subISR_table[a_count] ();
        }
    }
