...
FlagStatus SPI::MisoLevelGet() {
	bool remap_value = GPIO_isRemap [usedSPI];
//	return (FlagStatus)GPIO_ReadInputDataBit(MISO_ports[usedSPI][remap_value], MISO_pins[usedSPI][remap_value]);
	if(GPIO_ReadInputDataBit(MISO_ports[usedSPI][remap_value], MISO_pins[usedSPI][remap_value]) == SET) {
		return SET;
	}
	else return RESET;
}
